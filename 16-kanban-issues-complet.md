...
| **S5-21** | **`feat(api+client): résultats segmentés par profil de répondant + export impression/PDF (propositions et enquêtes)`** 🔶 *(enquêtes livrées ; reste : propositions + masquage des segments < 5 répondants avant export)* | **`lot:1` `type:feature` `zone:api` `zone:client`** | **`feat/results-segmented-export`** | **2 j** | **J62–J64** |
